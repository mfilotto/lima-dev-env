vault_installed:
  cmd.run:
    - name:
        ...
        && curl -LO https://releases.hashicorp.com/vault/{{ pillar['vault']['version'] }}/vault_{{ pillar['vault']['version'] }}_linux_amd64.zip 
        && unzip /root/vault_{{ pillar['vault']['version'] }}_linux_amd64.zip
        && mv /root/vault /usr/local/bin/vault
        && echo "complete -C /usr/local/bin/vault vault" > /etc/bash_completion.d/vault
        && rm /root/vault_{{ pillar['vault']['version'] }}_linux_amd64.zip
    - runas: root
    - unless:
